...
1. Render a sample invoice PDF with `render_invoice_pdf` (see `tests/test_invoice_pdf.py` for ready-made
   `CUSTOMER`/`SELLER`/`BASE_INVOICE` fixtures) and write the result to a file in the scratchpad directory. This
   exercises the real production path (HTML -> WeasyPrint PDF), not an approximation of it.
2. Rasterize the first page to PNG with `pypdfium2` (a `dev` dependency), e.g.:
   ```python
   import pypdfium2 as pdfium

   pdf = pdfium.PdfDocument("/path/to/preview.pdf")
   pdf[0].render(scale=2).to_pil().save("/path/to/preview.png")
   ```
